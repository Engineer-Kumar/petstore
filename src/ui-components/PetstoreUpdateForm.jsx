...
import { getPetstore } from "../graphql/queries";
import { updatePetstore } from "../graphql/mutations";
const client = generateClient();
export default function PetstoreUpdateForm(props) {
  const {
    id: idProp,
    petstore: petstoreModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = petstoreRecord
      ? { ...initialValues, ...petstoreRecord }
      : initialValues;
  ...
  const [petstoreRecord, setPetstoreRecord] = React.useState(petstoreModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPetstore.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPetstore
        : petstoreModelProp;
      setPetstoreRecord(record);
    };
    queryData();
  }, [idProp, petstoreModelProp]);
  React.useEffect(resetStateValues, [petstoreRecord]);
  const validations = {
    Name: [{ type: "Required" }],
    Age: [{ type: "Required" }],
    About: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePetstore.replaceAll("__typename", ""),
            variables: {
              input: {
                id: petstoreRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PetstoreUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || petstoreModelProp)}
